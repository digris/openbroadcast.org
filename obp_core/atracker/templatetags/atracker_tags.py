...
@register.inclusion_tag(
    "atracker/templatetags/events_for_object.html", takes_context=True
)
def events_for_object(context, obj):
    ...
    if events:
        return {"request": context["request"], "events": events}

    return {}


@register.inclusion_tag(
    "atracker/templatetags/events_for_object_by_verb.html", takes_context=True
)
def events_for_object_by_verb(context, obj, verb):

    events = Event.objects.by_obj(obj=obj).filter(event_type__title=str(verb))
    if events.exists():
...
@register.inclusion_tag(
    "atracker/templatetags/stats_for_object.html", takes_context=True
)
def stats_for_object(context, obj):
    statistics = summary_for_object(obj)
    if statistics:
        return {"request": context["request"], "statistics": statistics}
