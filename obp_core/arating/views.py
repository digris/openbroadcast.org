...
def vote(
    ...
    if isinstance(content_type, ContentType):
        ...
        raise ValueError(
            'content_type must be an instance of ContentType, a model, or "app.modelname" string'
        )

    # do the action
    # 404 if object to be voted upon doesn't exist
    if vote and content_type.model_class().objects.filter(pk=object_id).count() == 0:
        raise Http404
    ...
    if (
        vote
        and can_vote_test
        and not can_vote_test(request, content_type, object_id, vote)
    ):
