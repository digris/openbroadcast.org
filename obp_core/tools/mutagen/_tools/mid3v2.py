...
from mutagen._senf import fsnative, print_, argv, fsn2text, fsn2bytes, bytes2fsn
...
def list_frames(option, opt, value, parser):
    ...
    for name, frame in sorted(items):
        print_("    --{}    {}".format(name, frame.__doc__.split("\n")[0]))
    raise SystemExit
...
def delete_frames(deletes, filenames):
    ...
    for filename in filenames:
        with _sig.block():
            if verbose:
                print_("deleting %s from" % deletes, filename, file=sys.stderr)
            try:
...
def write_files(edits, filenames, escape):
    ...
    for frame, value in edits:
        ...
        try:
            ...
            error("{}: {}".format(frame, text_type(err)))
    ...
    for filename in filenames:
        with _sig.block():
            ...
            try:
                ...
                if verbose:
                    print_("No ID3 header found; creating a new tag", file=sys.stderr)
                id3 = mutagen.id3.ID3()
            except Exception as err:
...
def main(argv):
    ...
    parser.add_option(
        ...
        type="string",
        callback=lambda *args: args[3].edits.append((fsnative("--TPE1"), args[2])),
    )
    parser.add_option(
        "-A",
        "--album",
        ...
        type="string",
        callback=lambda *args: args[3].edits.append((fsnative("--TALB"), args[2])),
    )
    parser.add_option(
        "-t",
        "--song",
        ...
        type="string",
        callback=lambda *args: args[3].edits.append((fsnative("--TIT2"), args[2])),
    )
    parser.add_option(
        "-c",
        "--comment",
        ...
        type="string",
        callback=lambda *args: args[3].edits.append((fsnative("--COMM"), args[2])),
    )
    parser.add_option(
        "-p",
        "--picture",
        ...
        type="string",
        callback=lambda *args: args[3].edits.append((fsnative("--APIC"), args[2])),
    )
    parser.add_option(
        "-g",
        "--genre",
        ...
        type="string",
        callback=lambda *args: args[3].edits.append((fsnative("--TCON"), args[2])),
    )
    parser.add_option(
        "-y",
        "--year",
        ...
        type="string",
        callback=lambda *args: args[3].edits.append((fsnative("--TDRC"), args[2])),
    )
    parser.add_option(
        "-T",
        "--track",
        ...
        type="string",
        callback=lambda *args: args[3].edits.append((fsnative("--TRCK"), args[2])),
    )
