...
class Import(UUIDModelMixin, TimestampedModelMixin, models.Model):
    ...
    type = models.CharField(max_length=10, default=TYPE_WEB, choices=TYPE_CHOICES)
    ...
    def get_stats(self):
        ...
        stats["init"] = self.files.filter(status=ImportFile.STATUS_INIT)
        stats["done"] = self.files.filter(status=ImportFile.STATUS_DONE)
        stats["ready"] = self.files.filter(status=ImportFile.STATUS_READY)
        stats["working"] = self.files.filter(status=ImportFile.STATUS_PROGRESS)
        stats["warning"] = self.files.filter(status=ImportFile.STATUS_WARNING)
        stats["duplicate"] = self.files.filter(status=ImportFile.STATUS_DUPLICATE)
        stats["queued"] = self.files.filter(status=ImportFile.STATUS_QUEUED)
        stats["importing"] = self.files.filter(status=ImportFile.STATUS_IMPORTING)
        stats["error"] = self.files.filter(status=ImportFile.STATUS_ERROR)

        return stats

    def get_inserts(self):

        inserts = {"media_mb": [], "artist_mb": [], "release_mb": []}
    ...
    def get_importitems(self):
        ...
        for obj in [i for i in qs if i.content_object and i.content_type.model in cts]:
            ...
            importitems[ct]["items"].append(obj.content_object)

        return importitems

    def get_importitem_ids(self, ctype):
        ii_ids = ImportItem.objects.filter(
            content_type=ctype, import_session=self
    ...
    def identify_task(obj):
        ...
        if not obj.mimetype:
            try:
                ...
                mimetype = mime.from_file(obj.file.path)
...
def importitem_post_save(sender, instance, created, **kwargs):
    ...
    if (
        created
        and instance.import_session
        and instance.import_session.user
        and instance.import_session.collection_name
    ):
