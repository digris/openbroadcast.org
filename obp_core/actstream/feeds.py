...
class ObjectActivityFeed(Feed):
    ...
    def get_object(self, request, content_type_id, object_id):
        return get_object_or_404(
            ContentType, pk=content_type_id
    ...
    def link(self, obj):
        ...
        return reverse(
            "actstream_actor", None, (ContentType.objects.get_for_model(obj).pk, obj.pk)
        )
...
class ActivityStreamsFeed(AtomWithContentFeed):
    ...
    def add_item_elements(self, handler, item):
        ...
        if "actor" in item:
            ...
            handler.addQuickElement(
                "link",
                get_tag_uri(item["actor"].get_absolute_url(), None),
                {"type": "text/html"},
            )
        ...
        if "object" in item:
            ...
            handler.addQuickElement(
                "published", rfc3339_date(item["object_timestamp"]).decode("utf-8")
            )
            handler.addQuickElement(
                "link", item["object"].get_absolute_url(), {"type": "text/html"}
            )
        ...
        if "target" in item:
            ...
            handler.addQuickElement(
                "activity: object-type", str(item["target_object_type"])
            )
    ...
    def item_extra_kwargs(self, obj):
        ...
        if obj.target:
            try:
                ...
                target_id = f"{obj.target_content_type.model}/{obj.action_object.id}"
...
class ModelActivityFeed(Feed):
    ...
    def link(self, model):
        return reverse(
            "actstream_model", None, (ContentType.objects.get_for_model(model).pk,)
        )
