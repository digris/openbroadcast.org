...
const PlayerApp = Vue.extend({
  ...
  methods: {
    ...
    handle_action(action) {
      ...
      if (action.do === 'load') {
        ...
        APIClient.put(url, { items: action.items })
          .then((response) => {
            ...
            this.loading = false;
            const results = pre_process_loaded_items(response.data.results);
      ...
      if (action.do === 'play') {
        ...
        if (action.item) {
          ...
          const opts = {
            // url: 'http://obp-next.local:5000/media-asset/format/427d5dbc-6997-40a8-bebd-faa9a056ec7f/default.mp3',
    ...
    player_resume_blocked_autoplay() {
    ...
    player_play_offset(offset, media) {
      const all_media = [];
      this.itemsToPlay.forEach((item_to_play) => {
      ...
      const index = all_media.findIndex((element) => element.key === media.key) + offset;
