...
@register.inclusion_tag(
    "profiles/templatetags/mentor_for_user.html", takes_context=True
)
def mentor_for_user(context, profile, mentor):
    ...
    if profile.mentor == mentor and not profile.is_approved:
        ...
        notes = _(
            "You're mentoring this user. Approving his/her profile will give full platform-access to the respective user. Think twice before handling!\nPlease remember, only professionals (radio or music business) should be given access to the platform!"
        )
