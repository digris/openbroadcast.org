...
class UserResource(ModelResource):
    ...
    def dehydrate(self, bundle):
        ...
        bundle.data["groups"] = ",".join(
            bundle.obj.groups.values_list("name", flat=True)
        )
    ...
    def get_or_create_social_user(self, request, **kwargs):
        ...
        user_qs = get_user_model().objects.filter(
            social_auth__provider=provider, social_auth__uid=uid
        )
        ...
        if not user_qs.exists():
            ...
            remote_user = get_user_model().objects.create_user(
                username=username, email=user["email"], password=None
            )
            ...
            social_user = UserSocialAuth(
                user=remote_user, provider=provider, uid=uid, extra_data=extra_data
            )
            social_user.save()
