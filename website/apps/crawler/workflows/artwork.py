...
CRAWLER_HEADERS = {"User-Agent": "Open Broadcast Coverart Crawler 0.0.1"}
...
class ArtworkCrawler(object):
    ...
    _default_services = ["wikidata", "discogs"]
    ...
    def __init__(self, obj, services=None):
        ...
        preserved = Case(
            *[
                When(service=service, then=pos)
                for pos, service in enumerate(self.services)
            ]
        )
        self.relations = obj.relations.filter(service__in=self.services).order_by(
            preserved
        )
    ...
    def crawl_for_wikidata_image(self, url):
        ...
        image_url = "https://upload.wikimedia.org/wikipedia/commons/{}/{}/{}".format(
            md5[0:1], md5[0:2], urllib.quote_plus(image_name.encode("utf-8"))
        )
    ...
    def crawl_for_discogs_image(self, url):
        ...
        if not image_url:
            try:
                image_url = [i for i in data["images"] if i["type"] == "secondary"][0][
                    "uri"
                ]
            except (KeyError, IndexError):
                return
    ...
    def crawl_for_wikipedia_image(self, url):
        ...
        try:
            image_url = [
                t.find("img")
                for t in soup.findAll("table", {"class": "infobox vevent haudio"})
            ][0]["src"]
    ...
    def download_and_save_image(self, image_url):
        ...
        if os.path.isfile(_path_abs):
            type(self.obj).objects.filter(pk=self.obj.pk).update(main_image=_path)
