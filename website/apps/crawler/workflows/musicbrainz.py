...
def release_fetch_media_mb_ids(obj):
    ...
    if not qs_media.exists():
        ...
        for m in _data['media']:
            for t in m['tracks']:
                try:
                    ...
                    log.warning('unable to map tracknumber "{}" - {}'.format(t['number'], e))
        ...
        for m in qs_media:
            ...
            log.debug('track from results #{} - {}'.format(m.tracknumber, _track['title']))
...
class MBCrawler(object):
    ...
    def __init__(self, obj):
        ...
        log.debug('crawling metadata: {} - id:{} - mb_id:{}'.format(obj, obj.pk, self.mb_id))
