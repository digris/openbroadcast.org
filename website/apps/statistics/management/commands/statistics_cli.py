...
def label_statistics(scope, id, year, path):
    ...
    print('generate label statistics: scope: {} - id: {} - year: {} - output: {}'.format(
        scope, id, year, path)
    )
...
@click.option('-m', '--month', type=int, required=True)
@click.option('-p', '--path', type=click.Path(), required=False)
def suisa_statistics(channel_id, year, month, path):
    ...
    click.echo('generate label statistics: channel: {} - year: {} - month: {} - output: {}'.format(
