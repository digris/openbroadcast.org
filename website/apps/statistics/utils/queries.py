...
def get_media_for_label(label, start, end, event_type_id):
    ...
    log.debug(
        'get_media_for_label: "{}" - range: {} - {}, type: {}'.format(
            label, start, end, event_type_id
        )
    )
    ...
    qs = (
        ...
        .filter(
            ...
            events__created__range=[start, end],
        )
        .annotate(num_events=Count("events"))
        .select_related("release", "release__label", "artist")
        .prefetch_related(
            Prefetch(
                "events", queryset=Event.objects.filter(event_type__id=event_type_id)
            )
        )
        .order_by("release__name", "name")
    )
