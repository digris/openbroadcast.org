...
SCHEDULER_LOCK_AHEAD = getattr(
    settings, "SCHEDULER_LOCK_AHEAD", -60 * 60
)  # 1 minute, to allow caching of files
...
def schedule(request):
    ...
    data["range_filter"] = "&time_start__gte=%s&time_end__lte=%s&" % (
        range_start_s,
        range_end_s,
    )
    ...
    return render_to_response(
        "abcast/schedule.html", data, context_instance=RequestContext(request)
    )
...
def select_playlist(request):
    ...
    data = {"status": True, "playlist_id": playlist.id}
...
def schedule_object(request):
    ...
    if ct == "playlist":
        ...
        if not (obj.broadcast_status == 1 and obj.type == "broadcast"):
            ...
            return {
                "message": _(
                    "Sorry - this playlist does not meet all criterias to be scheduled."
                )
            }
    ...
    if es.count() > 0:
        ...
        try:
            message += u"<br>Emission schedule from %s to %s" % (
                time_start.time(),
                time_end.time(),
            )
            for conflicting_emission in es:
                message += u'<br> - overlaps "%s" - from %s to %s' % (
...
def copy_paste_day(request):
    ...
    if channel and source and target:
        ...
        # get emissions
        es = Emission.objects.filter(
            time_start__gte=source_start, time_end__lte=source_end, channel=channel
        )
        for e in es:
            ...
            # blocking before
            bloking_emissions = Emission.objects.filter(
                time_start__lte=e.time_start + offset,
                time_end__gte=e.time_start
                + offset
            ...
            # blocking after
            bloking_emissions = Emission.objects.filter(
                time_start__lte=e.time_end
                + offset
                - datetime.timedelta(seconds=OVERLAP_TOLERANCE),
                time_end__gte=e.time_end + offset,
                channel=channel,
            )
            if bloking_emissions.count() > 0:
                slot_free = False
...
def delete_day(request):
    ...
    if channel and date:
        ...
        # get emissions
        es = Emission.objects.filter(
            time_start__gte=time_start, time_end__lte=time_end, channel=channel
        )
    ...
    data = {"status": True, "deleted": emission_count}
