...
class ChannelResource(ModelResource):
    class Meta:
        ...
        filtering = {"created": ["exact", "range", "gt", "gte", "lt", "lte"]}
    ...
    def get_schedule(self, request, **kwargs):
        ...
        objects = scheduler.get_schedule(
            range_start=range_start, range_end=range_end, channel=channel
        )

        bundle = {"meta": {"total_count": len(objects)}, "objects": objects}
    ...
    def get_now_playing(self, request, **kwargs):
        ...
        es = Emission.objects.filter(
            channel=channel, time_start__lte=now, time_end__gte=now
        )
        ...
        if es.count() == 1:
            ...
            for item in items:
                ...
                item.time_start = (
                    ...
                    + datetime.timedelta(seconds=timeshift)
                )
                item.time_end = (
                    e_start
                    + datetime.timedelta(
                ...
                if item.time_start < now < item.time_end:
                    ...
                    start_next = (
                        item.time_end - now + datetime.timedelta(seconds=timeshift)
                    ).total_seconds()
    ...
    def get_program(self, request, **kwargs):
        ...
        for daypart in dayparts:
            ...
            emissions_qs = Emission.objects.filter(
                time_end__gt=time_start, time_start__lt=time_end, channel=channel
            ).order_by("-time_start")
    ...
    def get_dayparts(self, request, **kwargs):
        ...
        for daypart in dayparts:
            ...
            emissions = Emission.objects.filter(
                time_end__gte=time_start, time_start__lte=time_end, channel=channel
            ).order_by("-time_start")
...
class BaseResource(Resource):
    ...
    class Meta:
        ...
        authentication = MultiAuthentication(
            ApiKeyAuthentication(), SessionAuthentication(), Authentication()
        )
        authorization = Authorization()
