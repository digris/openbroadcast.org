...
class EmissionResource(ModelResource):
    ...
    content_object = GenericForeignKeyField(
        to=co_to, attribute="content_object", null=False, full=True
    )
    ...
    def apply_filters(self, request, applicable_filters):
        base_object_list = super(EmissionResource, self).apply_filters(
    ...
    def dehydrate(self, bundle):
        ...
        if obj.user:
            bundle.data["user"] = {
                ...
                "pseudonym": obj.user.profile.pseudonym,
                "display_name": obj.user.profile.get_display_name(),
            }
        ...
        if obj.content_object and obj.content_object.user:
            bundle.data["user_co"] = {
                ...
                "pseudonym": obj.content_object.user.profile.pseudonym,
                "display_name": obj.content_object.user.profile.get_display_name(),
            }
        ...
        try:
            ...
            profile_bundle = ProfileResource().build_bundle(
                obj=obj.content_object.user.profile, request=bundle.request
            )
    ...
    def reschedule(self, request, **kwargs):
        ...
        if (0 <= e.time_start.hour <= (SCHEDULER_OFFSET - 1)) and (
            0 <= e.time_start.hour <= (SCHEDULER_OFFSET - 1)
        ):
        ...
        if es.count() > 0:
            ...
            try:
                message += u'<br>Emission schedule "%s" - from %s to %s' % (
                    e.name,
                    time_start.time(),
                    time_end.time(),
                )
