...
class Emission(TimestampedModelMixin, UUIDModelMixin, models.Model):
    ...
    def get_api_list_url(self):
        return reverse(
            "api_dispatch_list",
            kwargs={"api_name": "v1", "resource_name": "abcast/emission"},
        )
    ...
    def has_lock(self):
        ...
        return self.time_start < datetime.datetime.now()
    ...
    def save(self, *args, **kwargs):
        ...
        if self.duration:
            self.time_end = self.time_start + datetime.timedelta(
...
def post_save_emission_task(obj):
    ...
    if obj.time_end > range_start and obj.time_start < range_end:
        ...
        data = scheduler.get_schedule_for_pypo(
            range_start=range_start, range_end=range_end
        )

        message = {"event_type": "update_schedule", "schedule": {"media": data}}
        pypo_send(message)
...
def pre_delete_emission(sender, **kwargs):
    ...
    if obj.time_end > range_start and obj.time_start < range_end:
        ...
        data = scheduler.get_schedule_for_pypo(
            range_start=range_start, range_end=range_end, exclude=[obj.pk]
        )
