...
class Autopilot(object):
    ...
    def add_emission(self, slot_start):
        ...
        if p:
            ...
            """
            hack!
            check if emission overlaps a day. in case that yes, just move it to the next day
            (day starts at 6am)
            """
            print '*****************'
            print 'hour start: %s' % e.time_start.hour
            print 'hour end: %s' % e.time_end.hour

            if e.time_start.hour < 6 and (e.time_end.hour >= 6 and e.time_end.minute > 0):
                e.time_start = e.time_start.replace(hour=6, minute=0, second=0)
                e.save()




            return round_dt(e.time_end, 300)
