...
def repair_durations(limit_range, dump_to, load_from, tolerance, log_file):
    ...
    if load_from:
        ...
        with click.progressbar(
            ...
            show_pos=True,
            width=48,
            label="Reprocessing {} tracks".format(len(item_ids)),
        ) as bar:
            ...
            for p in item.get_appearances():
                if not p.pk in affected_playlist_ids:
                    ...
                    affected_playlists.append(
                        {"obj": p, "current_duration": p.get_duration()}
                    )
...
def generate_master_hashes():
    ...
    # to save memory items are loaded from db individually
    values = (
        Media.objects.filter(master_sha1__isnull=True)
        .order_by("pk")
        .values("id")
        .nocache()
    )
    ...
    with click.progressbar(
        ...
        show_pos=True,
        width=48,
        label="Reprocessing {} hashes".format(len(item_ids)),
    ) as bar:
