...
OVERLAP_TOLERANCE = getattr(
    settings, "AUTOPILOT_OVERLAP_TOLERANCE", 5
)  # tolerance in seconds. keep low! (1-5 seconds)
...
class Autopilot(object):
    def __init__(self, channel_id, username):
        ...
        self.dayparts = BroadcastDaypart.objects.filter(
            daypartset__channel=self.channel, enable_autopilot=True
        ).order_by("time_start")
    ...
    def schedule(self, *args, **kwargs):
        ...
        log.info(
            'Scheduling: {:} days with offset {:} - on behalf of "{:}" on channel "{:}"'.format(
                days_fill, days_offset, self.user, self.channel
            )
        )
        ...
        exclude_qs = Emission.objects.filter(
            time_start__gt=range_start, time_end__lt=range_end, channel=self.channel
        )
        ...
        for i in range(days_fill):
            days_to_fill.append(day_start + datetime.timedelta(days=i))
    ...
    def schedule_day(self, day):
        ...
        log.info(
            "Added {:} emissions to scheduler on {:}".format(
                len(self.scheduled_emissions), self.channel.name
            )
        )
        ...
        while next_start and next_start < daypart["abs_time_end"]:
            ...
            if next_start:
                ...
                if qs.exists():
                    ...
                    log.info(
                        'Scheduling at {:} - pk: {:} "{:}"'.format(
                            next_start, playlist.pk, playlist.name
                        )
                    )
    ...
    def get_next_slot(self, time_start, time_end, min_duration=None, max_duration=None):
        ...
        log.debug(
            "Look for slot: {:} - {:} with durations min: {:} max: {:}".format(
        ...
        if ending_emissions.exists():
            ...
            log.debug(
                'ending emission "{:}" in range {:}-{:}, next start: {:}'.format(
        ...
        starting_emissions = (
            Emission.objects.filter(
                Q(
                    time_start__gte=next_start
                    - datetime.timedelta(seconds=OVERLAP_TOLERANCE)
                )
                & Q(time_start__lt=time_end),
                channel=self.channel,
            )
            .exclude(pk__in=[e.pk for e in ending_emissions])
            .order_by("time_start")
        )
        ...
        if starting_emissions.exists():
            ...
            if (
                starting_emission.time_start - next_start
            ).seconds < min_duration + OVERLAP_TOLERANCE:
            ...
            log.debug(
                'starting emission "{:}" in range {:}-{:}, next end: {:}'.format(
        ...
        while next_start and not slot_duration >= min_duration:
            log.debug(
                "slot duration of {:} is too short - requested: {:}".format(
    ...
    def get_daypart_qs(self, weekday, time_start, time_end):
        ...
        if time_start.hour >= 20:
            daypart_qs = PlaylistDaypart.objects.filter(
                day=weekday, time_start__gte=datetime.time(20, 0)
            )
    ...
    def get_playlist_qs(self, dayparts=None):
        ...
        playlists_qs = (
            ...
            .exclude(pk__in=self.playlist_ids_to_exclude)
            .nocache()
        )
    ...
    def reset(self, *args, **kwargs):
        ...
        if self.user:
            delete_qs = delete_qs.filter(user=self.user)
