...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "action", type=str, choices=["schedule", "reset", "test_slots"]
        )
    ...
    def handle(self, *args, **options):
        ...
        ap = Autopilot(channel_id=options["channel_id"], username=options["username"])

        ap_action = getattr(ap, options["action"])
        ap_action(days_offset=options["days_offset"], days_fill=options["days_fill"])
