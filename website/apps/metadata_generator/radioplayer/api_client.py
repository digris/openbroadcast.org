...
API_BASE_URL = getattr(settings, 'RADIOPLAYER_API_BASE_URL', 'https://ingest.swissradioplayer.ch')
...
class IngestAPIClient(object):
    ...
    def __init__(self):
        ...
        self.headers = {
            'user-agent': 'openbroadcast.org - radioplayer client/0.0.1',
        }
    ...
    def get(self, url, params=None):
        ...
        r = requests.get(self.compose_url(_url), params, auth=self.auth, headers=self.headers)
    ...
    def post(self, url, payload={}):
        ...
        _url = self.compose_url(url)
        payload.update({
            'rpId': self.station_id
        })

        log.info('API POST request: {} - {}'.format(_url, payload))

        r = requests.post(self.compose_url(_url), data=payload, auth=self.auth, headers=self.headers)
