...
SLIDE_BASE_IMAGE = getattr(
    settings,
    "DAB_SLIDE_BASE_IMAGE",
    os.path.join(os.path.dirname(os.path.dirname(__file__)), "asset", "slide_base.png"),
)

SLIDE_BASE_FONT = getattr(
    settings,
    "DAB_SLIDE_BASE_FONT",
    os.path.join(
...
class DABMetadataGenerator(object):
    ...
    the slide images are composed out of existing images (cover/artist picture etc) and text overlay.
    """
    ...
    def get_dl_plus(self):
        ...
        if (
            self.content_object
            and self.content_object.artist
            and self.content_object.release
        ):
            title = self.content_object.name
            artist = self.content_object.artist.name
    ...
    def get_slides(self):
        ...
        primary_text = t.render({"media": self.content_object})

        t = loader.get_template("metadata_generator/dab/slide_secondary_text.txt")
        secondary_text = t.render(
            {"emission": self.emission, "playlist": self.emission.content_object}
        )
        ...
        ######################################################################
        if (
            self.content_object.artist
            and self.content_object.artist.main_image
            and os.path.isfile(self.content_object.artist.main_image.path)
        ):
            slide = self.compose_image_slide(
                image_path=self.content_object.artist.main_image.path, slide_id=slide_id
            )
        ...
        if self.playlist.main_image and os.path.isfile(self.playlist.main_image.path):
            slide = self.compose_image_slide(
                image_path=self.playlist.main_image.path, slide_id=slide_id
            )
