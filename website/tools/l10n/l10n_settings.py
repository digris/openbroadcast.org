...
l10n_settings_defaults = {
    ...
    'currency_formats' : {
        'USD' : {'symbol': u'$', 'positive' : u"$%(val)0.2f",
                 'negative': u"-$%(val)0.2f", 'decimal' : '.'},
        'GBP' : {'symbol': u'£', 'positive' : u"£%(val)0.2f",
                 'negative': u"-£%(val)0.2f", 'decimal' : '.'},
    },
    
    'default_currency': 'USD',
    'show_admin_translations': True,  # Display in the admin
    'allow_translation_choice' : True, # Display the list of languages to store
                                       #  user
}
...
def get_l10n_setting(name, default_value = None):
    ...
    return settings.L10N_SETTINGS.get(
        name, l10n_settings_defaults.get(name, default_value))
...
def get_l10n_default_currency_symbol():
    ...
    try:
        ...
        log.warn('could not find default currency symbol, please make sure you '
                 'have a L10N_SETTINGS in your settings file, and that it has '
                 'a default currency.')
        return "$"
