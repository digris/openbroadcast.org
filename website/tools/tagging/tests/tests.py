...
class TestParseTagInput(TestCase):
    ...
    def test_with_comma_delimited_multiple_words(self):
        ...
        self.assertEqual(
            parse_tag_input("a-one, a-two and a-three"), ["a-one", "a-two and a-three"]
        )
    ...
    def test_tags_with_double_quotes_can_contain_commas(self):
        ...
        self.assertEqual(parse_tag_input('"two", one, one, two, "one"'), ["one", "two"])
        self.assertEqual(parse_tag_input('two", one'), ["one", "two"])
...
class TestTagUsageForModel(TestCase):
    ...
    def test_tag_usage_with_filter_on_model_objects(self):
        ...
        relevant_attribute_list = [
            (tag.name, hasattr(tag, "counts")) for tag in tag_usage
        ]
        self.assertEqual(len(relevant_attribute_list), 4)
...
class TestTagsRelatedForModel(TestCase):
    ...
    def test_related_for_model_with_tag_query_sets_as_input(self):
        ...
        related_tags = Tag.objects.related_for_model(
            Tag.objects.filter(name__in=["bar", "ter", "baz"]), Parrot, counts=True
        )
        relevant_attribute_list = [(tag.name, tag.count) for tag in related_tags]
        self.assertEqual(len(relevant_attribute_list), 0)
...
class TestTagCloudForModel(TestCase):
    ...
    def test_tag_cloud_for_model(self):
        ...
        relevant_attribute_list = [
            (tag.name, tag.count, tag.font_size) for tag in tag_cloud
        ]
        self.assertEqual(len(relevant_attribute_list), 4)
    ...
    def test_tag_cloud_for_model_filters(self):
        tag_cloud = Tag.objects.cloud_for_model(Parrot, filters={"state": "no more"})
        relevant_attribute_list = [
            (tag.name, tag.count, tag.font_size) for tag in tag_cloud
        ]
        self.assertEqual(len(relevant_attribute_list), 2)
...
class TestTagUsageForQuerySet(TestCase):
    ...
    def test_tag_usage_for_queryset(self):
        ...
        tag_usage = Tag.objects.usage_for_queryset(
            Parrot.objects.filter(Q(perch__size__gt=6) | Q(state__startswith="l")),
            counts=True,
        )
        relevant_attribute_list = [(tag.name, tag.count) for tag in tag_usage]
        self.assertEqual(len(relevant_attribute_list), 3)
        ...
        tag_usage = Tag.objects.usage_for_queryset(
            Parrot.objects.exclude(Q(perch__size__gt=6) | Q(perch__smelly=False)),
            counts=True,
        )
        relevant_attribute_list = [(tag.name, tag.count) for tag in tag_usage]
        self.assertEqual(len(relevant_attribute_list), 2)
        ...
        tag_usage = Tag.objects.usage_for_queryset(
            Parrot.objects.exclude(perch__smelly=True).filter(state__startswith="l"),
            counts=True,
        )
        relevant_attribute_list = [(tag.name, tag.count) for tag in tag_usage]
        self.assertEqual(len(relevant_attribute_list), 2)
...
class TestTagFieldInForms(TestCase):
    ...
    def test_recreation_of_tag_list_string_representations(self):
        ...
        self.assertEqual(
            edit_string_for_tags([plain, spaces, comma]), 'plain, spa ces, "com,ma"'
        )
...
@override_settings(
    ...
    TEMPLATE_LOADERS=("tagging.tests.utils.VoidLoader",),
)
class TestTaggedObjectList(TestCase):
    ...
    def get_view(
        ...
        if code == 200:
            ...
            self.assertEqual(len(response.context["object_list"]), expected_items)
            self.assertEqual(
                response.context["object_list"], response.context[friendly_context]
            )
