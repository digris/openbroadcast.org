...
def register(model, tag_descriptor_attr="tags", tagged_item_manager_attr="tagged"):
    """
    Sets the given model class up for working with tags.
    """
    if model in registry:
        raise AlreadyRegistered(
            "The model '%s' has already been registered." % model._meta.object_name
        )
