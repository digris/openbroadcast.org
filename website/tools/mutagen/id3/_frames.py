...
class Frame(object):
    ...
    def __init__(self, *args, **kwargs):
        if len(args) == 1 and len(kwargs) == 0 and isinstance(args[0], type(self)):
            ...
            for checker in self._framespec[len(args) :]:
                setattr(self, checker.name, kwargs.get(checker.name, checker.default))
    ...
    def _writeData(self, config=None):
        ...
        for writer in self._framespec:
            try:
                data.append(writer.write(config, frame, getattr(frame, writer.name)))
            except SpecError as e:
                raise error(e)
...
class NumericTextFrame(TextFrame):
    ...
    _framespec = [
        EncodingSpec("encoding", default=Encoding.UTF16),
        MultiSpec("text", EncodedNumericTextSpec("text"), sep=u"\u0000", default=[]),
    ]
...
class NumericPartTextFrame(TextFrame):
    ...
    _framespec = [
        EncodingSpec("encoding", default=Encoding.UTF16),
        MultiSpec(
            "text", EncodedNumericPartTextSpec("text"), sep=u"\u0000", default=[]
        ),
    ]
...
class ETCO(Frame):
    ...
    _framespec = [ByteSpec("format", default=1), KeyEventSpec("events", default=[])]
...
class SYTC(Frame):
    ...
    _framespec = [ByteSpec("format", default=1), BinaryDataSpec("data")]
...
class SYLT(Frame):
    ...
    def __str__(self):
        ...
        return u"\n".join(
            "[{0}{1}]: {2}".format(time, unit, text) for (text, time) in self.text
        )
...
class RVAD(Frame):
    ...
    _framespec = [RVASpec("adjustments", stereo_only=False)]
...
class PCNT(Frame):
    ...
    _framespec = [IntegerSpec("count", default=0)]
...
class PCST(Frame):
    ...
    _framespec = [IntegerSpec("value", default=0)]
...
class RBUF(Frame):
    ...
    * offset -- the location of the next ID3 tag, if any
    ...
    _framespec = [SizedIntegerSpec("size", size=3, default=0)]
...
class POSS(Frame):
    ...
    * position -- current position of the file
    ...
    _framespec = [ByteSpec("format", default=1), IntegerSpec("position", default=0)]
...
class SIGN(Frame):
    ...
    _framespec = [ByteSpec("group", default=0x80), BinaryDataSpec("sig")]
...
class SEEK(Frame):
    ...
    _framespec = [IntegerSpec("offset", default=0)]
...
class RVA(RVAD):
    ...
    _framespec = [RVASpec("adjustments", stereo_only=True)]
