...
from .._compat import text_type, chr_, PY3, swap_to_string, string_types, xrange
from .._util import (
    total_ordering,
    decode_terminated,
    enum,
...
class RVASpec(Spec):
    ...
    def write(self, config, frame, values):
        if len(values) < 2 or len(values) > self._max_values:
            raise SpecError(
                "at least two volume change values required, max %d" % self._max_values
            )
        ...
        byte_values = [
            BitPaddedInt.to_str(v, bits=8, width=-1, minwidth=2) for v in values
        ]
...
class MultiSpec(Spec):
    ...
    def _validate23(self, frame, value, **kwargs):
        if len(self.specs) != 1:
            return [
                [s._validate23(frame, v, **kwargs) for (v, s) in izip(val, self.specs)]
                for val in value
            ]
        ...
        if not isinstance(spec, EncodedTextSpec) or isinstance(spec, TimeStampSpec):
            return value
