...
class MPEGFrame(object):
    ...
    def __init__(self, fileobj):
        ...
        frame_length = (
            ((frame_size // 8 * self.bitrate) // self.sample_rate) + padding
        ) * slot
        ...
        if self.layer == 3:
            self._parse_vbr_header(fileobj, self.frame_offset, frame_size, frame_length)

        fileobj.seek(self.frame_offset + frame_length, 0)

    def _parse_vbr_header(self, fileobj, frame_offset, frame_size, frame_length):
        """Does not raise"""
...
class MP3(ID3FileType):
    ...
    def score(filename, fileobj, header_data):
        ...
        return (
            header_data.startswith(b"ID3") * 2
            + endswith(filename, b".mp3")
            + endswith(filename, b".mp2")
            + endswith(filename, b".mpg")
