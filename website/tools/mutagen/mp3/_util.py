...
class LAMEHeader(object):
    ...
    def guess_settings(self, major, minor):
        ...
        if self.vbr_method == 2:
            ...
            if self.vbr_method == 3:
                ...
                p = {(5, 32, 0): 7, (5, 8, 0): 8, (6, 8, 0): 9}.get(adjust_key, p)
                return u"-V %s" % p
    ...
    def parse_version(cls, fileobj):
        ...
        return (major, minor), u"%d.%d%s%s" % (major, minor, patch, flag_string), True
...
class XingHeader(object):
    ...
    def __init__(self, fileobj):
        ...
        try:
            self.lame_version, self.lame_version_desc, has_header = LAMEHeader.parse_version(
                fileobj
            )
            if has_header:
...
class VBRIHeader(object):
    ...
    def __init__(self, fileobj):
        ...
        if self.version != 1:
            raise VBRIHeaderError("Unsupported header version: %r" % self.version)
