...
from crispy_forms.bootstrap import AppendedPrependedText, AppendedText, PrependedText
...
class TestFormLayout(TestCase):
    ...
    def test_second_layout_multifield_column_buttonholder_submit_div(self):
        ...
        form_helper.add_layout(
            Layout(
                ...
                Column(
                    "first_name", "last_name", css_id="column_name", css_class="columns"
                ),
                ButtonHolder(
                    Submit(
                        "Save the world",
    ...
    def test_i18n(self):
        ...
        form_helper.layout = Layout(
            HTML(_("i18n text")), Fieldset(_("i18n legend"), "first_name", "last_name")
        )
    ...
    def test_default_layout(self):
        test_form = TestForm2()
        self.assertEqual(
            test_form.helper.layout.fields,
            [
                "is_company",
                "email",
                "password1",
...
class TestLayoutObjects(TestCase):
    def test_field_type_hidden(self):
        ...
        test_form.helper = FormHelper()
        test_form.helper.layout = Layout(
            Field("email", type="hidden", data_mierda=12), Field("datetime_field")
        )
...
class TestDynamicLayouts(TestCase):
    ...
    def test_get_field_names(self):
        ...
        self.assertEqual(
            layout_3.get_field_names(), [[[0, 0], "field_name"], [[1], "password"]]
        )

        layout_4 = Div(Div(Div("field_name")), Div("password"), "extra_field")
        self.assertEqual(
            layout_4.get_field_names(),
            [[[0, 0, 0], "field_name"], [[1, 0], "password"], [[2], "extra_field"]],
        )
    ...
    def test_layout_get_field_names(self):
        ...
        self.assertEqual(
            ...
            [[[0, 0], "field_name"], [[1], "password"], [[2, 0], "extra_field"]],
        )

        layout_3 = Layout(Div(Div(Div("email")), Div("password1"), "password2"))
        self.assertEqual(
            layout_3.get_field_names(),
            [[[0, 0, 0, 0], "email"], [[0, 1, 0], "password1"], [[0, 2], "password2"]],
        )
