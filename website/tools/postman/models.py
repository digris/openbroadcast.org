...
class MessageManager(models.Manager):
    ...
    def _last_in_thread(self):
        ...
        return (
            self.filter(thread__isnull=False)
            .values("thread")
            .annotate(models.Max("pk"))
            .values_list("pk__max", flat=True)
            .order_by()
        )
    ...
    def _folder(self, related, filters, option=None, order_by=None):
        ...
        if option == OPTION_MESSAGES:
            ...
            return qs.filter(
                models.Q(id__in=self._last_in_thread.filter(lookups))
                | models.Q(lookups, thread__isnull=True)
            ).extra(
                select={
                    "count": QUOTE_CHAR.join(
                        [
                            "SELECT COUNT(*) FROM ",
                            "postman_message",
                            " T" " WHERE T.",
    ...
    def inbox_unread_count(self, user):
        ...
        """
        return (
            self.inbox(user, related=False, option=OPTION_MESSAGES)
...
class Message(models.Model):
    ...
    moderation_status = models.CharField(
        _("status"), max_length=1, choices=STATUS_CHOICES, default=STATUS_PENDING
    )
    ...
    def _obfuscated_email(self):
        ...
        domain = bits[1]
        return "@".join(
            (shrunken_digest, domain.rsplit(".", 1)[0])
    ...
    def update_parent(self, initial_status):
        ...
        if self.moderation_status != initial_status:
            ...
            if self.is_accepted():
                ...
                if parent and (
                    not parent.replied_at or self.sent_at < parent.replied_at
                ):
    ...
    def notify_users(self, initial_status, is_auto_moderated=True):
        ...
        if initial_status == STATUS_PENDING:
            if self.is_rejected():
                ...
                (notify_user if self.recipient_id else email_visitor)(
                    self, "acceptance"
                )
    ...
    def get_moderation(self):
        ...
        return (
            ...
            self.moderation_by_id,
            self.moderation_date,
        ...
        if auto is None and percents:
            ...
            final_reason = ", ".join(
                [
                    ...
                    if r and not r.isspace() and percents[i] < 50
                ]
            )
            auto = average >= 50
        if auto is None:
