    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            is_successful = form.save(
                auto_moderators=auto_moderators, hacked_recipient=recipients
            )
            if is_successful:
                log.debug("successfully saved message")
                messages.success(
                    request, _("Message successfully sent."), fail_silently=False
                )
            else:
                log.warning("message rejected for at least one recipient")
                messages.warning(
                    request,
                    _("Message rejected for at least one recipient."),
                    fail_silently=False,
                )
            return redirect(
                request.GET.get("next", success_url or next_url or "postman_inbox")
            )
        ...
        if recipients:
            ...
            usernames = list(
                user_model.objects.values_list(user_model.USERNAME_FIELD, flat=True)
                .filter(
                    is_active=True,
                    **{
...
def reply(
    ...
    if request.method == "POST":
        ...
        form = form_class(
            post,
            sender=user,
            recipient=parent.sender or parent.email,
        ...
        if form.is_valid():
            ...
            if is_successful:
                messages.success(
                    request, _("Message successfully sent."), fail_silently=True
                )
            else:
                messages.warning(
                    request,
...
def _view(
    ...
    if msgs:
        ...
        for m in msgs:
            if not getattr(
                m, ("sender" if m.sender == user else "recipient") + "_archived"
            ):
                archived = False
                break
        else:
        ...
        return render_to_response(
            template_name,
            {
                ...
                "form": form_class(initial=received.quote(*formatters))
                if received
                else None,
...
def archive(request, *args, **kwargs):
    ...
    return _update(
        request,
        "archived",
        _("Messages or conversations successfully archived."),
        True,
        *args,
        **kwargs
    )
...
def delete(request, *args, **kwargs):
    ...
    return _update(
        request,
        "deleted_at",
        _("Messages or conversations successfully deleted."),
        now(),
        *args,
        **kwargs
    )
...
def undelete(request, *args, **kwargs):
    ...
    return _update(
        request,
        "deleted_at",
        _("Messages or conversations successfully recovered."),
        *args,
        **kwargs
    )
