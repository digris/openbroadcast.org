    ...
    def handle_noargs(self, **options):
        ...
        if verbose >= 1:
            self.stdout.write(
                ...
                + "Checking messages and conversations for inconsistencies...\n"
            )
        checks = [
            (
            ...
            (
                "Visitor's email is in excess.",
                Q(sender__isnull=False, recipient__isnull=False) & ~Q(email=""),
            ),
            (
                "Visitor's email is missing.",
                (Q(sender__isnull=True) | Q(recipient__isnull=True)) & Q(email=""),
            ),
            (
                "Reading date must be later to sending date.",
                Q(read_at__lt=F("sent_at")),
            ),
            (
                "Deletion date by sender must be later to sending date.",
                Q(sender_deleted_at__lt=F("sent_at")),
            ),
            (
                "Deletion date by recipient must be later to sending date.",
                Q(recipient_deleted_at__lt=F("sent_at")),
            ),
            (
                "Response date must be later to sending date.",
            ...
            (
                "Response date must be later to reading date.",
            ...
            (
                ...
                & (Q(parent__thread__isnull=True) | ~Q(parent__thread=F("thread"))),
            ),
        ]
        count = 0
        for c in checks:
