...
def format_body(sender, body, indent=_("> "), width=WRAP_WIDTH):
    ...
    """
    indent = force_unicode(
        indent
    )  # join() doesn't work on lists with lazy translation objects
    ...
    quote = "\n".join(
        [
            line.startswith(indent) and indent + line or wrapper.fill(line) or indent
...
def format_subject(subject):
    ...
    return (
        ...
        else str.format(subject=subject)
    )
...
def email_visitor(object, action):
    """Email a visitor."""
    email(
        "postman/email_visitor_subject.txt",
...
def notify_user(object, action):
    ...
    if action == "rejection":
        ...
        label = (
            "postman_reply"
            if (parent and parent.sender_id == object.recipient_id)
            else "postman_message"
        )
    else:
        return
    ...
    if notification:
        ...
        notification.send(
            users=[user],
            label=label,
            extra_context={"pm_message": object, "pm_action": action},
        )
    else:
        if not DISABLE_USER_EMAILING and user.email and user.is_active:
            email(
                "postman/email_user_subject.txt",
                "postman/email_user.txt",
                [user.email],
                object,
                action,
            )
