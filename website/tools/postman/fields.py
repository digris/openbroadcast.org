...
class BasicCommaSeparatedUserField(CharField):
    ...
    default_error_messages = {
        ...
        "max": _(
            "Ensure this value has at most {limit_value} distinct items (it has {show_value})."
        ),
        "min": _(
            "Ensure this value has at least {limit_value} distinct items (it has {show_value})."
        ),
    ...
    def clean(self, value):
        ...
        if unknown_names:
            errors.append(
                self.error_messages["unknown"].format(users=", ".join(unknown_names))
            )
        if self.user_filter:
            filtered_names = []
            for u in users[:]:
...
if app_name in settings.INSTALLED_APPS and arg_default:
    ...
    auto_complete_field = getattr(
        __import__(app_name + ".fields", globals(), locals(), [str(field_name)]),
        field_name,
    )
