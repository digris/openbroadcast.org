...
class Command(BaseCommand):
	option_list = BaseCommand.option_list + (
		...
		make_option('-n', '--natural-foreign', action='store_true', dest='use_natural_keys', default=False,
	...
	def handle(self, *app_labels, **options):
		...
		if len(app_labels) == 0:
			app_list = SortedDict((app, None) for app in filter(lambda a: True if a not in excluded_apps else False, apps.get_app_configs()))
		else:
			app_list = SortedDict()
