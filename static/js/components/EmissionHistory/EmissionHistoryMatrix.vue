...
export default {
  ...
  methods: {
    getEmissionsForRange(timeStart, timeEnd) {
      // add offset to catch short or overlapping entries
      timeStart = timeStart.add(3570, 'second');
      timeEnd = timeEnd.subtract(3570, 'second');

      return this.emissionHistory.filter((emission) => (dayjs(emission.timeStart).isBefore(timeStart) && dayjs(emission.timeEnd).isAfter(timeEnd)));
    },
    getSlotsForDay(day) {
    ...
    getLegend() {
      const day = dayjs(new Date()).utc().hour(DAY_START_HOUR).minute(0)
        .second(0)
        .millisecond(0);
      const slots = [];
